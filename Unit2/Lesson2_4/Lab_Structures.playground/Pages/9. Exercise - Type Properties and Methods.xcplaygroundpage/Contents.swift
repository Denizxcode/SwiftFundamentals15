 ...
 */
let user1 = User(userName: "Deniz", email: "deniz@outlook.com", age: 39)
User.logIn(user: user1)

// Print current user properties
if let currentUser = User.currentUser {
    print("Current User: \(currentUser.userName), \(currentUser.email), \(currentUser.age)")
}

// Log in with a different user
let user2 = User(userName: "Katie", email: "katie@outlook.com", age: 37)
User.logIn(user: user2)

// Print updated current user properties
if let currentUser = User.currentUser {
    print("Current User: \(currentUser.userName), \(currentUser.email), \(currentUser.age)")
}
/*:
